...
def run_propainter_waft(video, flow_masks, masks_dilated, output_folder, resize_ratio=1.0, height=-1, width=-1, ref_stride=10, neighbor_length=10, subvideo_length=80, raft_iter=20, mode='video_inpainting', scale_h=1.0, scale_w=1.2, save_fps=24, save_frames=True, fp16=True):
    ...
    save_root = os.path.join(output_folder, video_name)
    ...
    ##############################################
    # set up WAFT and flow competition model
    ##############################################
    ...
    with torch.no_grad():
        ...
        if frames.size(1) > short_clip_len:
            ...
            for f in range(0, video_length, short_clip_len):
                ...
                if f == 0:
                    flows_f, flows_b = fix_waft(frames[:,f:end_f], iters=raft_iter)
                else:
                    flows_f, flows_b = fix_waft(frames[:,f-1:end_f], iters=raft_iter)
            ...
            gt_flows_bi = fix_waft(frames, iters=raft_iter)
            torch.cuda.empty_cache()
...
def run_propainter(video, flow_masks, masks_dilated, output_folder, resize_ratio=1.0, height=-1, width=-1, ref_stride=10, neighbor_length=10, subvideo_length=80, raft_iter=20, mode='video_inpainting', scale_h=1.0, scale_w=1.2, save_fps=24, save_frames=True, fp16=True):
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    device = get_device()
