...
class Renderer:
    ...
    def render_fn(self, camera_state: CameraState, img_wh):
        # Handle both old tuple API and new RenderTabState API
        if not isinstance(img_wh, tuple):
        ...
        t = (
            int(img_wh.preview_time * (self.num_frames - 1))
            if canonical is None or not canonical.value
            else None
        )
        self.model.training = False
        img = self.model.render(t, w2c[None], K[None], (W, H))["img"][0]
        render_tracks = getattr(self.viewer, '_render_track_checkbox', None)
        if render_tracks is None or not render_tracks.value:
