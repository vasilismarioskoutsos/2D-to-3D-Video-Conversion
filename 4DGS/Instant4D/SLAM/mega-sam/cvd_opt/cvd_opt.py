...
if __name__ == "__main__":
  ...
  # Resize flows to match SLAM depth resolution after RESIZE_FACTOR
  target_h = int(disp_data.shape[-2] * RESIZE_FACTOR)
  target_w = int(disp_data.shape[-1] * RESIZE_FACTOR)
  scale_x = target_w / flows.shape[-1]
  scale_y = target_h / flows.shape[-2]
  flows = torch.nn.functional.interpolate(flows, size=(target_h, target_w), mode='bilinear', align_corners=False)
  flows[:, 0] *= scale_x
  flows[:, 1] *= scale_y
  if flow_masks.dim() == 3:
      flow_masks = torch.nn.functional.interpolate(flow_masks.unsqueeze(1), size=(target_h, target_w), mode='nearest').squeeze(1)
  elif flow_masks.dim() == 4:
      flow_masks = torch.nn.functional.interpolate(flow_masks, size=(target_h, target_w), mode='nearest')
  ...
  # Pure PyTorch SE3 inverse
  def _se3_inv_matrix(poses_data):
      t = poses_data[..., :3]
      q = poses_data[..., 3:]
      ...
      R = torch.stack([torch.stack([R00,R01,R02],-1),
                       torch.stack([R10,R11,R12],-1),
                       torch.stack([R20,R21,R22],-1)],-2)
      # w2c: R^T, -R^T @ t
      Rt = R.transpose(-1,-2)
      t_inv = -(Rt @ t.unsqueeze(-1)).squeeze(-1)
      B = poses_data.shape[0]
      mat = torch.eye(4, device=poses_data.device).unsqueeze(0).repeat(B,1,1)
      mat[:,:3,:3] = Rt
      mat[:,:3,3] = t_inv
      return mat
  ...
  for i in range(100):
    optim.zero_grad()
    cam_c2w = _se3_inv_matrix(poses_th)
