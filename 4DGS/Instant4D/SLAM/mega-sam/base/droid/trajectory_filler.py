...
class PoseTrajectoryFiller:
  ...
  def __fill(self, tstamps, images, intrinsics, depths=None, masks=None):
    ...
    poses_data = self.video.poses[:N].clone()

    t0 = torch.as_tensor([ts[ts <= t].shape[0] - 1 for t in tstamps])
    t1 = torch.where(t0 < N - 1, t0 + 1, t0)

    dt = (ts[t1] - ts[t0] + 1e-3).unsqueeze(-1)  # [M, 1]
    alpha = ((tt - ts[t0]) / (ts[t1] - ts[t0] + 1e-3)).unsqueeze(-1).clamp(0, 1)  # [M, 1]

    p0 = poses_data[t0]  # [M, 7]
    p1 = poses_data[t1]  # [M, 7]

    # Linear interpolation for translation
    t_interp = p0[:, :3] * (1 - alpha) + p1[:, :3] * alpha
    ...
    # Fall back to linear when theta~0
    linear_mask = (dot > 0.9995)
    w0 = torch.where(linear_mask, 1 - alpha, w0)
    w1 = torch.where(linear_mask, alpha, w1)
    q_interp = w0 * q0 + w1 * q1
    q_interp = q_interp / q_interp.norm(dim=-1, keepdim=True)

    gs_data = torch.cat([t_interp, q_interp], dim=-1).cuda()
    Gs = SE3(gs_data)
