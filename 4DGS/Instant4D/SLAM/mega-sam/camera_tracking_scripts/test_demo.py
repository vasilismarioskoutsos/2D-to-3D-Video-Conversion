...
import sys
sys.path.insert(0, r'C:\vasilis\2D-to-3D-Video-Conversion\4DGS\Instant4D\SLAM\mega-sam\base')
sys.path.insert(0, r'C:\vasilis\2D-to-3D-Video-Conversion\4DGS\Instant4D\SLAM\mega-sam\base\droid')
...
import faulthandler
faulthandler.enable(file=open('crash_dump.log', 'w'), all_threads=True)
...
def image_stream(
  ...
  for t, (image_file) in enumerate(image_list):
    ...
    # breakpoint()
    h0, w0, _ = image.shape
    h1 = int(h0 * np.sqrt((256 * 384) / (h0 * w0))) # original resolution 384 * 512
    
    w1 = int(w0 * np.sqrt((256 * 384) / (h0 * w0)))
...
if __name__ == "__main__":
  ...
  for t, (mono_disp_file, metric_depth_file) in enumerate(
    ...
    temp_mono = np.load(mono_disp_file, allow_pickle=True)

    if isinstance(temp_mono.item() if temp_mono.ndim == 0 else None, dict):
        da_disp = np.float32(temp_mono.item()['depth'])
    else:
        da_disp = np.float32(temp_mono)

    uni_data = np.load(metric_depth_file, allow_pickle=True)
  ...
  if len(result) == 3:
    traj_est, depth_est, motion_prob = result
  else:
    traj_est, depth_est = result
    motion_prob = np.zeros(1)
