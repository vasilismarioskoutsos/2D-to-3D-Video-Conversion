...
from utils.pose_utils import get_tensor_from_camera, quadmultiply
...
class GaussianModel:
    ...
    def get_scaling(self):
        # Use only the first scaling parameter for all XYZ dimensions
        return self.scaling_activation(self._scaling[:, 0:1]).repeat(1, 3)
    ...
    def get_scaling_xyzt(self):
        # Use shared scaling for XYZ (first parameter repeated 3 times) + separate time scaling
        shared_xyz_scaling = self.scaling_activation(self._scaling[:, 0:1]).repeat(1, 3)
        time_scaling = self.scaling_activation(self._scaling_t)
        return torch.cat([shared_xyz_scaling, time_scaling], dim = 1)
    ...
    def init_RT_seq(self, cam_list):
        poses =[]
        print(f"dtype of cam_list: {type(cam_list)}")
        for image , cam in cam_list:
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                group["params"][0] = nn.Parameter(group["params"][0][mask].requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]
        optimizable_tensors["motion"] = nn.Parameter(self.motion[mask].requires_grad_(True))
        return optimizable_tensors
    ...
    def prune_points(self, mask):
        ...
        self.motion = optimizable_tensors["motion"]
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                group["params"][0] = nn.Parameter(torch.cat((group["params"][0], extension_tensor), dim=0).requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]
        # print(f"tensors_dict['motion'].shape: {tensors_dict['motion'].shape}")
        # print(f"self.motion.shape: {self.motion.shape}")
        # raise Exception("stop here")
        optimizable_tensors["motion"] = nn.Parameter(torch.cat((self.motion, tensors_dict["motion"]), dim=0).requires_grad_(True))
        return optimizable_tensors

    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_t, new_scaling_t, new_rotation_r, new_motion):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, grads_t, grad_t_threshold, N=2):
        ...
        # Create new scaling by scaling down the first parameter only 
        selected_single_scaling = self.scaling_activation(self._scaling[selected_pts_mask, 0:1]) / (0.8*N)
        new_scaling = self._scaling[selected_pts_mask].repeat(N, 1)
        new_scaling[:, 0:1] = self.scaling_inverse_activation(selected_single_scaling.repeat(N, 1))
        new_rotation = self._rotation[selected_pts_mask].repeat(N,1)
        new_features_dc = self._features_dc[selected_pts_mask].repeat(N,1,1)
        new_features_rest = self._features_rest[selected_pts_mask].repeat(N,1,1)
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)
        new_motion_prob = self.motion[selected_pts_mask].repeat(N)
        if not self.rot_4d:
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_t, new_scaling_t, new_rotation_r, new_motion_prob)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, grads_t, grad_t_threshold):
        ...
        new_scaling_t = None
        new_rotation_r = None
        new_motion_prob = self.motion[selected_pts_mask]
        
        # print(f"--------------------new_motion_prob.shape: {new_motion_prob.shape}")
        # print(f"new rotation_r.shape: {new_rotation.shape}")
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_t, new_scaling_t, new_rotation_r, new_motion_prob)
