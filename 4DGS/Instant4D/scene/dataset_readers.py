...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        print(f"------{idx} timestamp: {idx/(len(cam_extrinsics)-1)}")
...
def fetchMeagSamNpz(path):
    metadata = np.load(path)
    positions = metadata['xyz']
    colors = metadata['rgb'] 
    motion = metadata['prob_motion']
    time_stamp = metadata['time_stamp']
    scale_t = metadata['scale_time']
    normals = np.zeros_like(positions)
    print(f"shape of colors: {colors.shape}")
    print(f"shape of positions: {positions.shape}")
    print(f"shape of motion: {motion.shape}")
    print(f"shape of time_stamp: {time_stamp.shape}")
    print(f"shape of scale_t: {scale_t.shape}")
    return BasicPointCloud(points=positions, colors=colors, normals=normals, time=time_stamp, motion=motion, scale_t=scale_t)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", time_duration=None, frame_ratio=1, dataloader=False):
    ...
    def frame_read_fn(idx_frame):
        ...
        print(f"------{idx} timestamp: {timestamp}")
        if frame_ratio > 1:
...
def readMegaSamSceneInfo(path, white_background, eval, extension=".png", num_pts=100_000, time_duration=None, num_extra_pts=0, frame_ratio=1, dataloader=False):
    print(f"Reading Mega Sam with voxel filtering")
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromCutter(path, "transforms_train.json", white_background, extension, time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader)
    print("Reading Test Transforms")
    test_cam_infos =  readCamerasFromCutter(path, "transforms_test.json" , white_background, extension, time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader)
    ...
    nerf_normalization = getNerfppNorm(train_cam_infos)
    npz_path = os.path.join(path, "filtered_cvd.npz")

    try:
        pcd = fetchMeagSamNpz(npz_path)
    except:
        raise ValueError(f"Could not find {npz_path}")
        # print error
        # print(f"Error: {e}")
...
def readIphoneSceneInfo(path, white_background, eval, extension=".png", num_pts=100_000, time_duration=None, num_extra_pts=0, frame_ratio=1, dataloader=False):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension, time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader)
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json" if not path.endswith('lego') else "transforms_val.json", white_background, extension, time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader)
