...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None,camera_pose=None):
    ...
    w2c_Identity = torch.eye(4).cuda()
    projmatrix = (
        w2c_Identity.unsqueeze(0).bmm(viewpoint_camera.projection_matrix.unsqueeze(0))
    ).squeeze(0)
    camera_pos = w2c_Identity.inverse()[3, :3]
    raster_settings = GaussianRasterizationSettings(
        ...
        viewmatrix=w2c_Identity, # use dummy view to let Pytorch calculate gradient for us 
        projmatrix=projmatrix,   # dummy projection
        sh_degree=pc.active_sh_degree,
        sh_degree_t=pc.active_sh_degree_t,
        campos=camera_pos,       # camera position
